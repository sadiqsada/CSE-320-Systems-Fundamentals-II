...
static int m_bishop[][2] = {
    {7, 7}, {6, 6}, {5, 5}, {4, 4}, {3, 3}, {2, 2}, {1, 1}, {7, -7}, {6, -6}, {5, -5}, {4, -4}, {3, -3}, {2, -2}, {1, -1}, {-7, -7}, {-6, -6}, {-5, -5}, {-4, -4}, {-3, -3}, {-2, -2}, {-1, -1}, {-7, 7}, {-6, 6}, {-5, 5}, {-4, 4}, {-3, 3}, {-2, 2}, {-1, 1}};

#define NB_M_ROOK 28
static int m_rook[][2] = {
    {7, 0}, {6, 0}, {5, 0}, {4, 0}, {3, 0}, {2, 0}, {1, 0}, {-7, 0}, {-6, 0}, {-5, 0}, {-4, 0}, {-3, 0}, {-2, 0}, {-1, 0}, {0, 7}, {0, 6}, {0, 5}, {0, 4}, {0, 3}, {0, 2}, {0, 1}, {0, -7}, {0, -6}, {0, -5}, {0, -4}, {0, -3}, {0, -2}, {0, -1}};

#define NB_M_QUEEN 56
static int m_queen[][2] = {
    {7, 7}, {6, 6}, {5, 5}, {4, 4}, {3, 3}, {2, 2}, {1, 1}, {7, -7}, {6, -6}, {5, -5}, {4, -4}, {3, -3}, {2, -2}, {1, -1}, {-7, -7}, {-6, -6}, {-5, -5}, {-4, -4}, {-3, -3}, {-2, -2}, {-1, -1}, {-7, 7}, {-6, 6}, {-5, 5}, {-4, 4}, {-3, 3}, {-2, 2}, {-1, 1}, {7, 0}, {6, 0}, {5, 0}, {4, 0}, {3, 0}, {2, 0}, {1, 0}, {-7, 0}, {-6, 0}, {-5, 0}, {-4, 0}, {-3, 0}, {-2, 0}, {-1, 0}, {0, 7}, {0, 6}, {0, 5}, {0, 4}, {0, 3}, {0, 2}, {0, 1}, {0, -7}, {0, -6}, {0, -5}, {0, -4}, {0, -3}, {0, -2}, {0, -1}};
...
#define setboard(A, I, J, P, C) \
  {                             \
    (A)->board[(I)][(J)] = (P); \
    (A)->color[(I)][(J)] = (C); \
  }
#define clsboard(A, I, J)        \
  {                              \
    (A)->board[(I)][(J)] = VOID; \
    (A)->color[(I)][(J)] = VOID; \
  }
...
#else
static int find_keyword(tab, nbentry, defaut, key, warning) char *tab[]; /* the table to look in */
...
#else
int path_free(l1, c1, l2, c2) int l1, c1, l2, c2;
#endif
{
  int li = 1;
  int ci = 1;
...
#else
int guess_depl(nb, tab, pl1, pc1, l2, c2, path) int nb;
int tab[10][2];
int *pl1, *pc1;
int l2, c2;
...
#endif
{
  ...
  if (tos->board[l2][c2] != VOID && (m->type != PRISE) && (m->type != PROM_ET_PRISE))
  {
  ...
  case PAWN:
    ...
    if ((c1 != c2) && (tos->board[l2][c2] == VOID) && (tos->board[l1][c2] == PAWN))
    {
      m->type = EN_PASSANT;
...
#else
int parse_number(token) char *token;
#endif
{
  int curmove = 0;
  int i;
...
#else
int parse_comment(com) char *com;
#endif
{
  int t;
...
#else
int parse_keyword(token, text) char *token;
char *text;
#endif
{
  char c;
...
#else
int parse_roque(token) char *token;
#endif
{
  int i;
...
#else
int parse_move(token) char *token;
#endif
{
  register int i;
  int correcte = FALSE;
...
#else
int parse_options(argc, argv) int argc;
char *argv[];
#endif
{
...
#else
int associe_traduction(table, language) char **table;
int language;
#endif
{
...
#else
int notation_main(argc, argv) int argc;
char *argv[];
#endif
{
