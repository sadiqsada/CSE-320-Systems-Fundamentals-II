...
#endif
{
  ...
  case D_ASCII:
    if (side != BLACK)
      (void)sprintf(d->white_buffer, "...");
    if (side != WHITE)
      (void)sprintf(d->black_buffer, "   ");
    break;
  case D_TEX:
    if (side != BLACK)
      (void)sprintf(d->white_buffer, "%s", FigDots);
    if (side != WHITE)
      (void)sprintf(d->black_buffer, "~");
    break;
  default:
...
#endif
{
      ...
      ((d->type != GRANDROQUE) && (d->type != PETITROQUE)))
  {
    ...
    themove[0] = '\0';
    if ((dr->output_move_format == SHORTENED) && (d->type == PRISE) && (d->piece == PAWN))
    ...
    if (dr->print_liaison)
    {
      ...
      else if ((dr->output_move_format == ALGEBRAIC))
        (void)sprintf(lie, "%c", '-');
    }
...
#endif
{
  ...
  if ((ftmp = fopen(strcpy(chaine, PS_HEADER), "r")) == NULL)
  ...
  /* footer file */
  (void)strcpy(chaine, LIB_DIR);
  if ((ftmp = fopen(strcpy(chaine, PS_FOOTER), "r")) == NULL)
